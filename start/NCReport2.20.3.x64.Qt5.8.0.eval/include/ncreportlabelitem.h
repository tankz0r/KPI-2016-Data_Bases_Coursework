...
public:
    NCReportLabelData()
     : wordbreak(false), rotation(0), hideRepeated(false), hasTranslation(false), hasCondFormat(false), fillchar(QChar(' '))
    {}
...
public:
    ...
    { return ((NCReportLabelData*)d)->fillchar; }

    inline void setFillChar( const QChar& c )
    { ((NCReportLabelData*)d)->fillchar = c; }
