...
public:
    ...
    enum ReportType { Report=0, TextDocumentPrint, Label };
    enum LabelDirection { Horizontal=0, Vertical };
    ...
    /*! Specifies the report source type */
    void setReportSourceType( NCReportSource::ReportSourceType type );

    void setReportSourceFactory( NCReportSourceFactory* factory );
    NCReportSourceFactory* reportSourceFactory() const;

    /*! Specifies the report source object */
    void setReportSource( NCReportSource *reportSource);
    ...
    void addItemView( const QString& id, QAbstractItemView* itemView );
#endif
    ...
    NCReportPrinter::PrinterMode printQuality() const;
    int printResolution() const;
    void setPrintQuality( NCReportPrinter::PrinterMode );
    void setPrintResolution(int);
#ifndef NCREPORT_NO_PREVIEW_WINDOW
    NCReportPreviewWindow *createPreviewWindow( QWidget* parent = 0);
    QWidget* runReportToShowPreview(QWidget *parent = 0, bool maximized = false);
    bool runReportToShowPreviewDonePrint();
    QWidget* execPreview(NCReportOutput* output, QWidget *parent=0, bool maximized = false);
...
public slots:
    ...
    void runReportToPdf(const QString& filename );
    void runReportToPdfWriter(const QString& filename, int dpi = 600);
    void runReportToPdfWriter(QIODevice* device, int dpi = 600);
...
private slots:
    void emitDataSourceOpen( const QString& dsID );
    void emitDataSourceOpened( const QString& dsID, int totalRows );
    void emitDataRowProgress( const QString& dsID, int row );
    void emitDataRowProgress( int row );
    void emitDataRowCount(int count );
    void emitPageProgress( int page );
    void onSectionProgress( const QString& sectionID );
...
private:
    ...
    void createReportSource();
    bool fileIsWritable( const QString& fileName) const;
};
