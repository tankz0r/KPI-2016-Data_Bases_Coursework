...
public:
    NCReportItemData()
    : objectType(0), typeId(0x0), sourceType(0), fitRole(0), editorMode(false),
    posMM(QPointF(0,0)), offsetMM(QPointF(0,0)), sizeMM(QSizeF(0,0)), realSizeMM(QSizeF(0,0)), sectionSizeMM(QSizeF(0,0)), zoneSizeMM(QSizeF()),
    editSize(QSizeF(0,0)), dynPosMM(QPointF(0,0)), dynSizeMM(QSizeF(0,0)),
    dynPosEnabled(false), dynSizeEnabled(false),
    ...
    pinToRight(false), pinToLeft(false), dynOffset(false),
    ...
    QPointF posMM;
    QPointF offsetMM;		// offset pos for Zone handling
    QSizeF  sizeMM;			// fixed size in mm
    QSizeF  realSizeMM;		// real or required size in mm (for Label, Text)
    QSizeF  sectionSizeMM;	// size of the current section
    QSizeF  zoneSizeMM;		// size of the current zone (inside the section)
    QSizeF  editSize;
    QPointF dynPosMM;         // dynamic position in mm
    QSizeF dynSizeMM;         // dynamic size in mm
    bool dynPosEnabled;       // dynamic position is enabled
    bool dynSizeEnabled;      // dynamic size is enabled
    ...
    bool pinToRight;
    bool pinToLeft;
    bool dynOffset;     // dynamic position is offset (relative) mode
    ...
    QString htmlOptions;
    QString functionId;
    QString dynPosXSource;
    QString dynPosYSource;
    QString dynWidthSource;
    QString dynHeightSource;
...
public:
    ...
    enum FitRole {
        NoFit=0,
        FitHeightToSection = 1,
        FitHeightToZone = 2,
        FitHeightToPageFooter = 3
    };
    ...
    */
    ...
    void setEditorMode( bool emode )
    { d->editorMode = emode; }
    inline bool editorMode() const
    { return d->editorMode; }
    ...
    void setDynamicPosXSource( const QString& source )
    { d->dynPosXSource = source; }
    inline QString dynPosXSource() const
    { return d->dynPosXSource; }

    void setDynamicPosYSource( const QString& source )
    { d->dynPosYSource = source; }
    inline QString dynPosYSource() const
    { return d->dynPosYSource; }

    void setDynamicWidthSource( const QString& source )
    { d->dynWidthSource = source; }
    inline QString dynWidthSource() const
    { return d->dynWidthSource; }

    void setDynamicHeightSource( const QString& source )
    { d->dynHeightSource = source; }
    inline QString dynHeightSource() const
    { return d->dynHeightSource; }

    void setDynamicOffsetMode( bool offset )
    { d->dynOffset = offset; }
    inline bool dynamicOffsetMode() const
    { return d->dynOffset; }
    ...
    */
    ...
    virtual bool evaluateProperty(NCReportEvaluator *evaluator , const QString &property, QString& result);
    ...
    virtual bool isValidLocation( int sectionType ) const;
    virtual void adjustFont( QFont & font, NCReportOutput* output = 0 ) const;
